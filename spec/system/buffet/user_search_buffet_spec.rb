...
describe 'Usuário dono de buffet' do
  ...
  it 'não consegue acessar rota de buscar' do
    # Arrange
    user = BuffetOwnerUser.create!(email: 'gustavo@email.com', password: 'password', name: 'Gustavo')
    # Act
    login_as user, :scope => :buffet_owner_user
    visit search_buffets_path
    # Assert
    expect(current_path).not_to eq search_buffets_path
    expect(current_path).to eq root_path
    expect(page).to have_content 'Você como dono de buffet, apenas pode gerenciar seu buffet'
  end
